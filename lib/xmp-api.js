...
function updateJPEGXMP(buffer, { title, description, tags = [] }) {
  ...
  const tagXML = tags.map(t => `<rdf:li>${t}</rdf:li>`).join("")
...
<x:xmpmeta xmlns:x="adobe:ns:meta/">
 <rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
  ...
  const length = header.length + xmpBytes.length + 2

  const newSegment = new Uint8Array(4 + header.length + xmpBytes.length)

  newSegment[0] = 0xff
  newSegment[1] = 0xe1
  newSegment[2] = (length >> 8) & 0xff
  newSegment[3] = length & 0xff

  newSegment.set(header, 4)
  newSegment.set(xmpBytes, 4 + header.length)
  ...
  while (offset < jpeg.length) {
    ...
    if (marker === 0xda) { // Start of scan
      segments.push(jpeg.slice(offset))
      break
    }

    const size = (jpeg[offset + 2] << 8) + jpeg[offset + 3]
    const segment = jpeg.slice(offset, offset + size + 2)

    if (marker === 0xe1) {
      const content = decoder.decode(segment.slice(4, 35))
  ...
  if (!xmpFound) {
    segments.splice(0, 0, jpeg.slice(0, 2))
    segments.splice(1, 0, newSegment)
    segments.push(jpeg.slice(2))
    return concatSegments(segments)
  }

  segments.unshift(jpeg.slice(0, 2))
  return concatSegments(segments)

  function concatSegments(parts) {
